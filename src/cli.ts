...
// Borrow code from https://github.com/soheilpro/mailwind/blob/master/src/index.js for original mailwind.
...
const argv = yargs(process.argv)
  ...
  .describe(
    "tailwind-config",
    "The path to your custom Tailwind config file"
  ).argv;
...
async function main() {
  ...
  if (!argv["output-css"] && !argv["output-html"]) {
    console.log(
      "Either --output-css or --output-html options must be specified."
    );
    process.exit(1);
    return;
  }
  ...
  if (
    argv["input-css"] &&
    argv["output-css"] &&
    path.resolve(argv["input-css"]) === path.resolve(argv["output-css"])
  ) {
    console.log(
      "The --input-css and --output-css options cannot refer to the same file."
    );
    process.exit(1);
    return;
  }
  ...
  if (
    argv["input-html"] &&
    argv["output-html"] &&
    path.resolve(argv["input-html"]) === path.resolve(argv["output-html"])
  ) {
    console.log(
      "The --input-html and --output-html options cannot refer to the same file."
    );
    process.exit(1);
    return;
  }
  ...
  const tailwind_config_path =
    argv["tailwind-config"] || path.resolve(__dirname, "./tailwind.config.js");
  const input_css_path =
    argv["input-css"] || path.resolve(__dirname, "./style.css");
  const output_css_path =
    argv["output-css"] || path.resolve(os.tmpdir(), "mailwind.css");
  const input_html_path = argv["input-html"];
  const output_html_path = argv["output-html"];

  const result: any = await exec(process.argv0, [
    tailwindcss_path,
    "--config",
